...
class UserRepository @Inject()(dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) {
  ...
  def getByEmailAndPassword(email: String, password: String): Future[Option[User]] = db.run {
    user.filter(user => user.email === email && user.password === password).result.headOption
  }
