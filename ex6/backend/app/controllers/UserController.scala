...
class UserController @Inject()(val userRepository: UserRepository, cc: MessagesControllerComponents)(implicit ec: ExecutionContext) extends MessagesAbstractController(cc){
    ...
    def getUserByEmailAndPassword(email: String, password: String): Action[AnyContent] = Action.async {
      val user = userRepository.getByEmailAndPassword(email,password)
      user.map {
        case Some(res) => Ok(Json.toJson(res))
        case None => NotFound("There is no user with that email and password")
      }
    }
